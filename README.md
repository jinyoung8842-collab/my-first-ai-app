...
안티그래비티가 코드를 만들어주면 그대로 적용합니다. 패키지 설치 명령(`npm install @google/genai`)도 함께 알려줄 거예요. 모델 이름은 가끔 바뀌니, 안 되면 [Gemini 모델 목록](https://ai.google.dev/gemini-api/docs/models)에서 지금 쓸 수 있는 Flash-Lite 모델로 바꿔 부탁하세요(2026년 9월 기준). 무료 등급으로 보낸 내용은 구글 제품 개선에 쓰일 수 있으니 개인정보는 넣지 마세요.
